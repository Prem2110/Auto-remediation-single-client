...
var getScopedVarName = (name, withVersion = false) => {
  if (!withVersion) {
    return name;
  }
...
var locationOpen = (url, target, features2) => {
  if (isSSR) {
    return;
  }
  window.open(url, target, features2);
};
...
var attachConfigurationReset = (listener) => {
  eventProvider2.attachEvent(CONFIGURATION_RESET, listener);
};
var resetConfiguration = () => {
  eventProvider2.fireEvent(CONFIGURATION_RESET, void 0);
};
...
var getThemeRoot = () => {
  initConfiguration();
  if (initialConfig.themeRoot === void 0) {
    return;
  }
  if (!validateThemeRoot_default(initialConfig.themeRoot)) {
    console.warn(`The ${initialConfig.themeRoot} is not valid. Check the allowed origins as suggested in the "setThemeRoot" description.`);
    return;
  }
...
var attachThemeLoaded = (listener) => {
  eventProvider4.attachEvent(THEME_LOADED, listener);
};
var detachThemeLoaded = (listener) => {
  eventProvider4.detachEvent(THEME_LOADED, listener);
};
...
var FontFace_css_default = `@font-face{font-family:"72";font-style:normal;font-weight:400;src:url(https://cdn.jsdelivr.net/npm/@sap-theming/theming-base-content@11.33.0/content/Base/baseLib/baseTheme/fonts/72-Regular.woff2) format("woff2"),local("72");unicode-range:U+00,U+0D,U+20-7E,U+A0-FF,U+131,U+152-153,U+161,U+178,U+17D-17E,U+192,U+237,U+2C6,U+2DC,U+3BC,U+1E9E,U+2013-2014,U+2018-201A,U+201C-201E,U+2020-2022,U+2026,U+2030,U+2039-203A,U+2044,U+20AC,U+2122}
...
var insertScrollbarStyles = () => {
  if (!hasStyle("data-ui5-scrollbar-styles")) {
    createStyle(ScrollbarStyles_css_default, "data-ui5-scrollbar-styles");
  }
};
...
var internals2 = {
  ...
  get chrome() {
    ...
    return /(Chrome|CriOS)/.test(internals2.userAgent);
  },
  get firefox() {
    if (isSSR3) {
      return false;
    }
    return /Firefox/.test(internals2.userAgent);
  },
  get safari() {
    if (isSSR3) {
      return false;
    }
    return !internals2.chrome && /(Version|PhantomJS)\/(\d+\.\d+).*Safari/.test(internals2.userAgent);
  },
  get webkit() {
    if (isSSR3) {
      return false;
    }
    return /webkit/.test(internals2.userAgent);
  },
  get windows() {
    if (isSSR3) {
      return false;
  ...
  get iOS() {
    ...
    return !!navigator.platform.match(/iPhone|iPad|iPod/) || !!(internals2.userAgent.match(/Mac/) && "ontouchend" in document);
  },
  get android() {
    if (isSSR3) {
      return false;
    }
    return !internals2.windows && /Android/.test(internals2.userAgent);
  },
  get androidPhone() {
    if (isSSR3) {
      return false;
    }
    return internals2.android && /(?=android)(?=.*mobile)/i.test(internals2.userAgent);
  },
  get ipad() {
    if (isSSR3) {
      return false;
    }
    return /ipad/i.test(internals2.userAgent) || /Macintosh/i.test(internals2.userAgent) && "ontouchend" in document;
  },
  _isPhone() {
    detectTablet();
    return internals2.touch && !tablet;
  }
};
var windowsVersion;
var webkitVersion;
...
var isWindows8OrAbove = () => {
  ...
  if (!internals2.windows) {
    return false;
  }
  if (windowsVersion === void 0) {
    const matches = internals2.userAgent.match(/Windows NT (\d+).(\d)/);
...
var isWebkit537OrAbove = () => {
  ...
  if (!internals2.webkit) {
    return false;
  }
  if (webkitVersion === void 0) {
    const matches = internals2.userAgent.match(/(webkit)[ /]([\w.]+)/);
...
var detectTablet = () => {
  ...
  if (internals2.touch) {
    ...
    if (internals2.chrome && internals2.android) {
      tablet = !/Mobile Safari\/[.0-9]+/.test(internals2.userAgent);
      return;
    }
    let densityFactor = window.devicePixelRatio ? window.devicePixelRatio : 1;
    if (internals2.android && isWebkit537OrAbove()) {
      densityFactor = 1;
    }
    tablet = Math.min(window.screen.width / densityFactor, window.screen.height / densityFactor) >= 600;
    return;
  }
  tablet = internals2.userAgent.indexOf("Touch") !== -1 || internals2.android && !internals2.androidPhone;
};
var supportsTouch = () => internals2.touch;
var isSafari = () => internals2.safari;
var isChrome = () => internals2.chrome;
var isFirefox = () => internals2.firefox;
var isTablet = () => {
  detectTablet();
  return (internals2.touch || isWindows8OrAbove()) && tablet;
};
var isPhone = () => {
  return internals2._isPhone();
};
var isDesktop = () => {
  if (isSSR3) {
    return false;
...
var isIOS = () => {
  return internals2.iOS;
};
var isMac = () => {
  return internals2.macOS;
};
var isAndroid = () => {
  return internals2.android || internals2.androidPhone;
};
...
var attachBoot = (listener) => {
  if (!booted) {
    eventProvider5.attachEvent("boot", listener);
    return;
  }
  listener();
};
...
var detachLanguageChange = (listener) => {
  eventProvider6.detachEvent(LANG_CHANGE, listener);
};
...
//# sourceMappingURL=chunk-I62KTPLT.js.map
