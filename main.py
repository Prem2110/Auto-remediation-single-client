...
async def aem_status():
    """Return AEM connectivity info, observer queue depth (via SEMP), and pipeline stage counts."""
    aem_enabled = os.getenv("AEM_ENABLED", "false").lower() == "true"
    queue_name  = os.getenv("AEM_OBSERVER_QUEUE", "sap.cpi.autofix.observer.out")
    rest_url    = os.getenv("AEM_HOST", os.getenv("AEM_REST_URL", ""))
    semp_host   = os.getenv("AEM_SEMP_HOST", "")
    semp_user   = os.getenv("AEM_SEMP_USERNAME", "")
    semp_pass   = os.getenv("AEM_SEMP_PASSWORD", "")
    vpn         = os.getenv("AEM_VPN", "sierra-aws-service1")

    queue_depth: Optional[int] = None
    semp_error:  Optional[str] = None

    if semp_host and semp_user:
        try:
            url = f"{semp_host.rstrip('/')}/SEMP/v2/monitor/msgVpns/{vpn}/queues/{queue_name}"
            async with httpx.AsyncClient(timeout=3, verify=False) as client:
                resp = await client.get(url, auth=(semp_user, semp_pass))
            if resp.status_code == 200:
                data = resp.json().get("data", {})
                queue_depth = (
                    data.get("msgs", {}).get("spooledCount")
                    or data.get("spooledMsgCount")
                    or 0
                )
            else:
                semp_error = f"SEMP HTTP {resp.status_code}"
        except Exception as exc:
            semp_error = str(exc)[:120]

    # Single COUNT(*) — no row data fetched
...
async def aem_webhook(event: Dict[str, Any]):
    """
    SAP AEM REST Delivery Point (RDP) webhook.

    Solace pushes queue messages here via HTTPS POST.  The body is the raw
    message JSON — no wrapper, no 'topic' field required.
    ...
    CPI error events that have no stage).
    """
    if orchestrator is None:
        raise HTTPException(status_code=503, detail="Orchestrator not ready")
    asyncio.create_task(orchestrator._route_stage(event))
    logger.info("[AEM] Webhook received, dispatched to _route_stage")
