# Security — SAP CPI Self-Healing Agent
...
- `.env` is in `.gitignore` and must **never** be committed. Use `.env.vector_store.example` as the pattern for documenting required variables.
- On SAP BTP Cloud Foundry, bind secrets via VCAP_SERVICES / user-provided service instances — do not pass them as CF environment variables in `manifest.yml`.
...
- The FastAPI server currently trusts the `user_id` field in request bodies (internal use).
...
- MCP server URLs are stored in the `MCP_SERVERS` dict in `main.py` — do not hardcode them in router files.
...
- Set explicit timeouts (60 s) on all MCP HTTP transports — never use `timeout=None`.

---

## Autonomous Loop Safety

- The autonomous fix+deploy loop (`_autonomous_loop`) must only run when `AUTONOMOUS_ENABLED=true` is explicitly set.
- `AUTO_FIX_CONFIDENCE` (default 0.90) gates autonomous deployment — do not lower this threshold without explicit user approval.
- Every autonomous action (retry, fix, deploy) must be written to the `AUTONOMOUS_INCIDENTS` table with actor `"autonomous"` before the action executes.
